...
parser.add_argument('--valid_ratio', type=float, default=2e-2)
parser.add_argument('--batch_size', type=int, default=16)
parser.add_argument('--lr', type=float, default=1e-2)
parser.add_argument('--train_savepath', type=str, default='./results/power/')
parser.add_argument('--n_train_steps', type=int, default=int(30000))
parser.add_argument('--n_steps_per_epoch', type=int, default=int(1e3))
parser.add_argument('--sw_dir', type=str, default='./runs/power/')
parser.add_argument('--sw_name', type=str, default='debug')
parser.add_argument('--resample', type=int, default=0)
parser.add_argument('--horizon', type=int, default=8)
parser.add_argument('--test_ratio', type=float, default=1e-2)
parser.add_argument('--no_cond', type=int, default=0)
parser.add_argument('--data_name', type=str, default='power_18_9_31_100000_sigma=0.01')
parser.add_argument('--retrain_data_name', type=str, default='power_18_9_31_100000_sigma=0.01')
...
parser.add_argument('--resample_num', type=int, default=1000)
...
parser.add_argument('--use_clustering', type=int, default=0)
parser.add_argument('--use_smoothness', type=str, default='uni_first')
parser.add_argument('--n_timesteps', type=int, default=64)
...
# data_name = f'power_{n}_{m}_{T-1}_{N}_sigma={sigma}.pkl'
# data_dict = {}
# data_dict['sys'] = { "C": np.identity(n)}
# # data_dict['adj'] = np.copy(A)
# data_dict['meta_data'] = {'num_nodes': n, 'input_dim': m, 'control_horizon': T-1, 'num_samples': N}
# data_dict['data'] = {'X0': X0,  'Y_bar': Y_bar, 'Y_f': Y_f}
...
if args.free_guide:
    if args.use_invdyn:
        if args.use_end_second:
            if args.use_smoothness not in ['first', 'second']:
                model = EndTemporalUnetFreeSecond(transition_dim=p, cond_dim=n, denoiser_cond_dim=1, dim=32, dim_mults=(1, 4), attention=False)
            else:
                model = EndTemporalUnetFreeSecond(transition_dim=p, cond_dim=n, denoiser_cond_dim=T+1, dim=32, dim_mults=(1, 4), attention=False)
        ...
        if args.has_invdyn and args.use_end_second:
            model = EndTemporalUnetInvdynFreeSecond(transition_dim=m+p, action_dim=m, cond_dim=p, dim=32, dim_mults=(1, 4), attention=False)
        elif args.has_invdyn and args.use_end:
            model = EndTemporalUnetInvdynFree(transition_dim=m+p, action_dim=m, cond_dim=p, dim=32, dim_mults=(1, 4), attention=False)
        elif args.has_invdyn:
            model = TemporalUnetInvdynFree(transition_dim=m+p, action_dim=m, cond_dim=p, dim=32, dim_mults=(1, 4), attention=False)
        elif args.use_end:
            raise NotImplementedError
    ...
    # assert not args.use_invdyn
        if args.has_invdyn and args.use_end:
            model = EndTemporalUnetInvdyn(transition_dim=m+p, action_dim=m, cond_dim=p, dim=32, dim_mults=(1, 8), attention=False)
        elif args.has_invdyn:
            model = TemporalUnetInvdyn(transition_dim=m+p, action_dim=m, cond_dim=p, dim=32, dim_mults=(1, 8), attention=False)
        elif args.use_attn:
            model = CondTemporalUnet(transition_dim=m+p, cond_dim=p, dim=32, dim_mults=(1, 8), attention=False)
        elif args.use_end:
            model = EndTemporalUnet2(transition_dim=m+p, cond_dim=p, dim=32, dim_mults=(1, 8), attention=False)
        else:
            model = TemporalUnet(transition_dim=m+p, cond_dim=p, dim=32, dim_mults=(1, 8), attention=False)
    if not args.resample:
        diffusion = GaussianDiffusionClassifierGuided(model, horizon=T+1, observation_dim=p, action_dim=m, n_timesteps=64, loss_type='l2', clip_denoised=False, predict_epsilon=args.pred_eps, action_weight=1., loss_discount=1.0, loss_weights=None, scale=args.scale, inv_dyn=args.use_invdyn, use_lambda=args.use_lambda, repaint=args.repaint)
    else:
        raise NotImplementedError
...
if args.resample and args.normalized:
    ...
    if args.free_guide:
        train_data = TrainData_norm_free(U_3d[:num_train], Y_bar_3d[:num_train], Y_f_3d[:num_train], kuramoto=True, train=True, use_clustering=args.use_clustering, use_smoothness=args.use_smoothness,normalizer='identity_normalizer')
        val_data = TrainData_norm_free(U_3d[num_train:num_train+num_val], Y_bar_3d[num_train:num_train+num_val], Y_f_3d[num_train:num_train+num_val], kuramoto=True, train=True, use_clustering=args.use_clustering, use_smoothness=args.use_smoothness,normalizer='identity_normalizer')
    else:
...
if args.sample_use_test:
    if args.normalized:
        if args.free_guide:
            test_data = TrainData_norm_free(U_3d[-num_test:], Y_bar_3d[-num_test:], Y_f_3d[-num_test:], kuramoto=True, use_clustering=args.use_clustering,normalizer='identity_normalizer')
        else:
...
for i in range(args.loops):
    ...
    if args.regen:
        ...
        samples_Y_bar_list = []
        samples_Y_f_list = []
        for i,actions in enumerate(samples_U):
            observations = env.from_actions_to_obs_direct(actions, start=samples_Y_bar[i,0])
    ...
    if args.normalized:
        if args.free_guide:
            train_data = TrainData_norm_free(samples_U[:], samples_Y_bar[:], samples_Y_f[:], kuramoto=True, train=True, use_clustering=args.use_clustering, use_smoothness=args.use_smoothness)
