...
class TrainData_norm(Dataset):
    def __init__(self, U, Y_bar, Y_f, normalizer='identity_normalizer', zero_pad=False ,train=False):
        super().__init__()
        assert len(U.shape)==3 and len(Y_bar.shape)==3 and len(Y_f.shape)==2 # N, T, m; N, T-1, p; N, p
        U_tensor = torch.tensor(U, dtype=torch.float32)
        U_zeros = torch.zeros(U_tensor.shape[0], 1, U_tensor.shape[2])
    ...
    def __getitem__(self, index):
        ...
        # cond = {0: y[0]}
        # cond={}
        return Batch2(x, cond, 1)
    
    def __len__(self):
        return self.U.shape[0]
    
class TrainData_norm_free(Dataset):
    def __init__(self, U, Y_bar, Y_f, normalizer='identity_normalizer', zero_pad=False, train=False, use_clustering=True, use_smoothness=None):
        super().__init__()
        assert len(U.shape)==3 and len(Y_bar.shape)==3 and len(Y_f.shape)==2 # N, T, m; N, T-1, p; N, p
        U_tensor = torch.tensor(U, dtype=torch.float32)
        U_zeros = torch.zeros(U_tensor.shape[0], 1, U_tensor.shape[2])
        ...
        # # print the distribution of U, Y
        # mean_U = torch.mean(self.U)
        # std_U = torch.std(self.U)
        # mean_Y = torch.mean(self.Y)
        # std_Y = torch.std(self.Y)
        # print(f'mean_U: {mean_U}, std_U: {std_U}, mean_Y: {mean_Y}, std_Y: {std_Y}')
