...
class GaussianDiffusionClassifierFree(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        train_conditioning=True, use_lambda = False, repaint=False, env=None, inv_hid=64, node_dim=None, node_f_dim=None,
    ):
        super().__init__()
        self.horizon = horizon
        self.observation_dim = observation_dim
        ...
        if not inv_dyn:
            ...
            self.action_dim = action_dim
            if node_dim==None:
                self.node_dim=observation_dim + action_dim
            else:
                self.node_dim=node_dim
            self.transition_dim = self.node_dim*self.node_f_dim
        else:
            self.real_action_dim = action_dim
            if node_dim==None:
                self.node_dim=observation_dim
            else:
                self.node_dim=node_dim
            self.action_dim = self.node_dim*self.node_f_dim-observation_dim
        ...
        if inv_dyn:
            self.inv_model = ARInvModel(hidden_dim=inv_hid, observation_dim=observation_dim, action_dim=action_dim)
            # self.inv_model = TransformerInvModel2(hidden_dim=64, observation_dim=observation_dim, action_dim=action_dim)
            # self.inv_model = GTModel(observation_dim=observation_dim, action_dim=action_dim)
        betas = cosine_beta_schedule(n_timesteps)
        alphas = 1. - betas
        alphas_cumprod = torch.cumprod(alphas, axis=0)
        alphas_cumprod_prev = torch.cat([torch.ones(1), alphas_cumprod[:-1]])
    ...
    def q_sample(self, x_start, t, noise=None, cond=None):
        ...
        if cond is not None:
            mask_cond = torch.zeros_like(x_start)
            for key, value in cond.items():
                mask_cond[..., key, :] = torch.ones_like(x_start[..., key, :]).to(x_start.device)
                noise[mask_cond.bool()] = 0 # mask out the noise for the conditioned part
        sample = (
            extract(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start +
            extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * noise
        )
        ...
        if cond is not None:
            sample[mask_cond.bool()] = x_start[mask_cond.bool()] # mask out the noise for the conditioned part
    ...
    def p_losses(self, x_start, cond, denoiser_cond, t, mask=None):
        ...
        if not self.repaint:
            if not self.predict_epsilon:
                # x_recon = apply_conditioning(x_recon, cond, self.action_dim)
                x_recon[mask_cond.bool()] = x_start[mask_cond.bool()] # mask out the noise for the conditioned part
            else:
                x_recon[mask_cond.bool()] = noise[mask_cond.bool()] # mask out the noise for the conditioned part
    ...
    def loss(self, x, *args):
        ...
        if self.use_inv_dyn:
            ...
            diffuse_loss, info = self.p_losses(x[...,(self.real_action_dim-self.action_dim):], *args, t, mask=mask)
            info['diffuse_loss'] = diffuse_loss.item()
            info['inv_loss'] = inv_loss.item()
...
class GaussianDiffusionClassifierGuidance(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        loss_type='l2', clip_denoised=False, predict_epsilon=True,
        action_weight=1.0, loss_discount=1.0, loss_weights=None, scale=0.1, inv_dyn=False, 
        use_lambda = False, repaint=False, env=None, inv_hid=64
    ):
        super().__init__()
        self.horizon = horizon
        self.sample_horizon = horizon if horizon==2**int(np.log2(horizon)) else 2**(int(np.log2(horizon))+1)
        self.observation_dim = observation_dim
        ...
        self.model = model
        self.transition_dim = self.model.transition_dim
        
        # self.use_lambda = use_lambda
        self.repaint = repaint
        if inv_dyn:
            self.inv_model = ARInvModel(hidden_dim=inv_hid, observation_dim=observation_dim, action_dim=action_dim)
            # self.inv_model = MLPInvModel(input_dim=observation_dim, output_dim=action_dim)
            # self.inv_model = TransformerInvModel2(hidden_dim=64, observation_dim=observation_dim, action_dim=action_dim)
            # self.inv_model = GTModel(observation_dim=observation_dim, action_dim=action_dim)
        betas = cosine_beta_schedule(n_timesteps)
        alphas = 1. - betas
        ...
        #     loss_weights = loss_weights.unsqueeze(0).repeat(16,1,1)
        # print(loss_weights)
        self.loss_fn = Losses[loss_type](loss_weights, self.transition_dim-self.observation_dim)
        self.scale = scale
        self.env = env
        self.resample_num=1
    ...
    def get_loss_weights(self, action_weight, discount, weights_dict):
        ...
        ## decay loss with trajectory timestep: discount**t
        discounts = discount ** torch.arange(self.sample_horizon, dtype=torch.float)
        discounts = discounts / discounts.mean()
        loss_weights = torch.einsum('h,t->ht', discounts, dim_weights)
    ...
    def p_mean_variance(self, x, t, cond, denoiser_cond):
        ...
        t_inp = t
        x_model_in = x
        x_recon = self.predict_start_from_noise(x_tmp, t=t, noise=self.model(x_model_in, cond, denoiser_cond, t_inp))
    ...
    def guidance(self, x, num_samp=1, return_grad_of=None):
        ...
        with torch.enable_grad():
            ...
            # compute losses and gradient
            # use inverse dynamics model to get the action
            if self.use_inv_dyn:
                x_shifted = torch.cat([x[:, :self.horizon-1, :], x[:, 1:self.horizon, :]], dim=-1)
                shape_x = x_shifted.shape
                x_shifted = x_shifted.reshape(-1, shape_x[-1])
                action = self.inv_model(x_shifted)
                action = action.reshape(shape_x[0], shape_x[1], -1)
                action = torch.cat([action, torch.zeros_like(action[:, -1:, :])], dim=1)
                x_action = torch.cat([action, x[:,:self.horizon]], dim=-1)
            else:
                x_action = x[:, :self.horizon+1]
            tol_loss, loss_dict, loss_by_sample = self.current_guidance(x_action)
            # print(tot_loss)
            tol_loss.backward()
            guide_grad = x.grad if return_grad_of is None else return_grad_of.grad
    ...
    def p_sample(self, x, t, cond, denoiser_cond, returns=None, apply_guidance=True, guide_clean=False, eval_final_guide_loss=True):
        ...
        b, *_, device = *x.shape, x.device
        with with_func():
            model_mean, _, model_log_variance, q_posterior_in = self.p_mean_variance(x=x, t=t, cond=cond, denoiser_cond=denoiser_cond)
        # noise = 0.5*torch.randn_like(x)
        noise = torch.randn_like(model_mean)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 1)))
        ...
        if guide_clean:
            # perturb clean trajectory
            guided_clean = q_posterior_in[0] - guide_grad*self.scale*sigma
            # use the same noisy input again
            guided_x_t = q_posterior_in[1]
            # re-compute next step distribution with guided clean & noisy trajectories
            model_mean, _, _ = self.q_posterior(x_start=guided_clean,
            ...
            x_out = model_mean - guide_grad*self.scale*sigma + noise
        if eval_final_guide_loss:
            # eval guidance loss one last time for filtering if desired
            #       (even if not applied during sampling)
            _, guide_rewards, guide_sample = self.guidance(x_out.clone().detach().requires_grad_(), num_samp=1)
    ...
    def p_sample_loop(self, shape, cond, denoiser_cond, returns=None, verbose=True, return_diffusion=False, apply_guidance=True,
        ...
        if conds.shape[0]==1:
            conds = conds.repeat(batch_size, 1, 1)
        if not self.repaint:
            x = apply_conditioning(x, cond, self.transition_dim-self.observation_dim)
        ...
        # x = apply_conditioning(x, cond, self.action_dim)
        if return_diffusion: diffusion = [x]
        pb = tqdm(reversed(range(0, self.n_timesteps)), desc='sampling loop time step', total=self.n_timesteps)
        guidance_losses = {}
        for i in pb:
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            if self.repaint: 
                for j in range(self.resample_num+1):
                    x, guide_rewards, guide_sample = self.p_sample(x=x, t=timesteps, cond=conds, denoiser_cond=denoiser_cond, returns=returns,
                                                                apply_guidance=apply_guidance,
                                                                guide_clean=guide_clean,
                                                                eval_final_guide_loss=(i == 0) and (j==self.resample_num))
                    if not self.repaint:
                        x = apply_conditioning(x, cond, self.transition_dim-self.observation_dim)
                    else:
                        if timesteps.max() == 0:
                            ...
                            for i, c in enumerate(cond.keys()):
                                conds_diffuse_dict[c] = conds_diffuse[:, i, :]
                        x = apply_conditioning(x, conds_diffuse_dict, self.transition_dim-self.observation_dim)
                    if self.resample_num:
                        # diffuse back to timesteps
            ...
            for key, value in guide_rewards.items():
                if key not in guidance_losses:
                    guidance_losses[key] = []
                guidance_losses[key].append(value.mean().item())
            if 'timesteps' not in guidance_losses:
                guidance_losses['timesteps'] = []
            guidance_losses['timesteps'].append(i)
        ...
        return Sample(x, guide_sample, diffusion, guide_rewards), guidance_losses
    ...
    def p_losses(self, x_start, cond, denoiser_cond, t, mask=None):
        ...
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise, cond=cond)
        # if not self.repaint:
            # x_noisy = apply_conditioning(x_noisy, cond, self.transition_dim-self.observation_dim)
        ...
        if not self.repaint:
            if not self.predict_epsilon:
                # x_recon = apply_conditioning(x_recon, cond, self.transition_dim-self.observation_dim)
                x_recon[mask_cond.bool()] = x_start[mask_cond.bool()] # mask out the noise for the conditioned part
            else:
                x_recon[mask_cond.bool()] = noise[mask_cond.bool()] # mask out the noise for the conditioned part
    ...
    def diffuse_loss(self, x, *args):
        ...
        if self.use_inv_dyn:
            ...
            # a_t = x[:, :-1, :self.real_action_dim]
            # x_t_1 = x[:, 1:, self.real_action_dim:]
            # x_comb_t = torch.cat([x_t, x_t_1], dim=-1).detach()
            # x_comb_t = x_comb_t.reshape(-1, 2 * self.observation_dim)
            # a_t = a_t.reshape(-1, self.real_action_dim)
            # inv_loss = torch.functional.F.mse_loss(self.inv_model(x_comb_t), a_t)
            x_diff = x[...,(self.real_action_dim-self.action_dim):].detach()
            mask = torch.ones_like(x_diff).to(x.device)
            # if self.use_lambda:
                # if args[1].ndim==1 or args[1].shape[-1]==1:
                    # mask = mask * torch.exp(0.5*(args[1].reshape(-1, *([1]*(x.ndim-args[1].ndim)))-1))
            
            diffuse_loss, info = self.p_losses(x_diff, *args, t, mask=mask)
            info['diffuse_loss'] = diffuse_loss.item()
            # info['inv_loss'] = inv_loss.item()
            ...
            #     mask[:, -1, :self.real_action_dim] = 0
            #     lamb=0.98
            #     lamb_power_t = lamb**t.float()
    def loss(self, x, *args):
        batch_size = len(x)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
        if self.use_inv_dyn:
            x_t = x[:, :self.horizon-1, self.real_action_dim:]
            a_t = x[:, :self.horizon-1, :self.real_action_dim]
            x_t_1 = x[:, 1:self.horizon, self.real_action_dim:]
            x_comb_t = torch.cat([x_t, x_t_1], dim=-1).detach()
            x_comb_t = x_comb_t.reshape(-1, 2 * self.observation_dim)
            a_t = a_t.reshape(-1, self.real_action_dim)
            inv_loss = torch.functional.F.mse_loss(self.inv_model(x_comb_t), a_t)
            x_diff = x[...,(self.real_action_dim+self.observation_dim-self.transition_dim):].detach()
            mask = torch.ones_like(x_diff).to(x.device)
            mask[:,self.horizon:,:] = 0
            mask[:,self.horizon-1,:self.transition_dim-self.observation_dim] = 0 # mask last action which is not used
            # if self.use_lambda:
            #     if args[1].ndim==1 or args[1].shape[-1]==1:
            #         mask = mask * torch.exp(0.5*(args[1].reshape(-1, *([1]*(x.ndim-args[1].ndim)))-1))
            ...
            x_diff = x[...,(self.real_action_dim+self.observation_dim-self.transition_dim):].detach()
            mask = torch.ones_like(x_diff).to(x.device)
            mask[:,-1,:self.transition_dim-self.observation_dim] = 0 # mask last action which is not used
            # if self.use_lambda:
            #     if args[1].ndim==1 or args[1].shape[-1]==1:
            #         mask = mask * torch.exp(0.5*(args[1].reshape(-1, *([1]*(x.ndim-args[1].ndim)))-1))
...
class ARInvModel(nn.Module):
    def __init__(self, hidden_dim, observation_dim, action_dim, low_act=-1.0, up_act=1.0):
        ...
        print(f'Num of parameters of arinv model: {num_params}')
...
class MLPInvModel(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(MLPInvModel, self).__init__()
        self.input_dim = input_dim
        # choose appropriate hidden_dim according to the input_dim and output_dim
        hidden_dim1 = max(2^int(np.log2(2*input_dim + output_dim))*8, 256)
        hidden_dim2 = max(2^int(np.log2(2*input_dim + output_dim))*8, 256)
        hidden_dim3 = max(2^int(np.log2(2*input_dim + output_dim))*4, output_dim*2)
        self.hidden_dim1 = hidden_dim1
        self.hidden_dim2 = hidden_dim2
        self.hidden_dim3 = hidden_dim3
        self.output_dim = output_dim

        self.fc1 = nn.Linear(2 * input_dim, hidden_dim1)
        ...
        num_params = sum(p.numel() for p in self.parameters())
        print(f'Num of parameters of mlpinv model: {num_params}')
...
class GTModel(nn.Module):
    ...
    def forward(self, comb_state):
        ...
        for i in range(self.action_dim):
            action = state2[...,i]-state1[...,i]-0.01*state1[...,i+9] 
