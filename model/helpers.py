...
class Downsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.Conv1d(dim, dim, 3, 2, 1) #COMM: set stride=2 to downsample
...
class Upsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.ConvTranspose1d(dim, dim, 4, 2, 1) #COMM: L' = (L-1)*stride - 2*padding + kernel_size, so here L'=2*L
...
def extract(a, t, x_shape): #COMM: extract t^th timestep of a, and broadcast to x_shape
    b, *_ = t.shape
    out = a.gather(-1, t)
