...
class CrossAttention(nn.Module):
    ...
    def flash_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        :param q: are the query vectors before splitting heads, of shape `[batch_size, seq(t), d_attn]`
        :param k: are the query vectors before splitting heads, of shape `[batch_size, seq(d_cond), d_attn]`
        :param v: are the query vectors before splitting heads, of shape `[batch_size, seq(d_cond), d_attn]`
        """
    ...
    def normal_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        # Reshape to `[batch_size, t, n_heads * d_head]`
        out = out.reshape(*out.shape[:2], -1)
        # Map to `[batch_size, t, d_model]` with a linear layer
        return self.to_out(out)
